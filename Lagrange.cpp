...
Lagrangian::LAGRANGIAN_STATUS Lagrangian::runSubGradientMethod(list<pair<int, int>> &_lagFixationToOne, list<pair<int, int>> &_lagFixationToZero, double _bestKnownUB)
{
    ...
    while ((iterCounter++ < maxIterLag) && (alpha > eps))
    {
        ...
        ub = lagrangianHeuristic(prob, 20); //Heuristic();
...
double Lagrangian::lagrangianHeuristic(double _rclParam, double _sizeList)
{
    double solCost = 0.;
    int sentinel = 0;
    ...
    localSearch(&heuristicSolution[0]);
    for (int i = 0; i < g->nb_nodes; i++)
...
void Lagrangian::localSearch(int * _Solution)
{
    vector< vector <int> > aux;

    aux.resize(g->nb_nodes, vector<int>());

    for (int node = 0; node < g->nb_nodes; node++)
    {
        if (_Solution[node] != -1)
        {
            aux[node].push_back(node);
            for (int neighbour = 0; neighbour < vColorNeighborhood[node][_Solution[node]].size(); neighbour++)
            {
                aux[vColorNeighborhood[node][_Solution[node]][neighbour]].push_back(node);
            }
        }
    }

    vector <int > dominating_number(g->nb_nodes, g->nb_nodes);

   for (int node = 0; node < g->nb_nodes; node++)
    {
        for (int l = 0; l < aux[node].size(); l++)
        {
            int a = aux[node][l];
            int b = aux[node].size();
            dominating_number[a] = (dominating_number[a] < b ? dominating_number[a] : b);
        }
    }
 
    vector <int > uselessNodes;

    for (int node = 0; node < g->nb_nodes; node++)
    {
        if (dominating_number[node] >= 2 && dominating_number[node] < g->nb_nodes && !isVarFixedToOne(node))
    ...
    if (uselessNodes.size() != 0)
    {
        ...
        int bad_node = uselessNodes[0];

        for (int node = 1; node < uselessNodes.size(); node++)
        {
            if (g->nodesVec[uselessNodes[node]]->weights[_Solution[uselessNodes[node]]] >
                    g->nodesVec[bad_node]->weights[_Solution[bad_node]])
                bad_node = uselessNodes[node];
        }

        _Solution[bad_node]=  - 1;

        localSearch(_Solution);
    }
}

int Lagrangian::change_color_node(int * _Solution)
{
    int bad_node = -1;
    int value = INFINITY;
    ...
    for (int node = 0; node < g->nb_nodes; node++)
    {
        if (_Solution[node] != -1 && g->nodesVec[node]->colors.size() >= 2 && !isVarFixedToOne(node))
        {
            if (g->nodesVec[node]->weights[_Solution[node]] < value)
            {
                bad_node = node;
                value = g->nodesVec[node]->weights[_Solution[node]];
            }
        }
    }

    if (bad_node == -1) return -1;

    int old_color = _Solution[bad_node];

    for (int color = 0; color < g->nodesVec[bad_node]->colors.size(); color++)
        if (g->nodesVec[bad_node]->colors[color] != _Solution[bad_node])
        {
            _Solution[bad_node] = g->nodesVec[bad_node]->colors[color];
            break;
        }

    if (_Solution[bad_node] == old_color) return -1;

    for (int color = 0; color < g->nodesVec[bad_node]->colors.size(); color++)
        if (g->nodesVec[bad_node]->colors[color] != _Solution[bad_node] && g->nodesVec[bad_node]->colors[color] != old_color)
            if (g->nodesVec[bad_node]->weights[color] < g->nodesVec[bad_node]->weights[_Solution[bad_node]])
            {
                _Solution[bad_node] = g->nodesVec[bad_node]->colors[color];
            }

    for (int neighbour = 0; neighbour < vColorNeighborhood[bad_node][old_color].size(); neighbour++)
    {
        if (_Solution[vColorNeighborhood[bad_node][old_color][neighbour]] == -1)
        {
            _Solution[vColorNeighborhood[bad_node][old_color][neighbour]] = old_color;
        }
    }
...
/*************************************************************/
int const compare(const C_assignment * pi1, C_assignment * pi2)
{
    return (pi1->weight > pi2->weight);
}

double Lagrangian::Heuristic()
{
    ...
    for (int i = 0; i < g->nb_nodes; ++i)
    {
        for (int c = 0; c < g->nodesVec[i]->colors.size(); ++c)
        {
            if (isVarFixedToZero(i, c))
                continue;

            if (isVarFixedToOne(i) && Sol_nodes_tmp[i]->color != -1)
            {
                int _c = fixedToOneVars[i];
                Sol_nodes_tmp[i]->color = _c;
                Sol_nodes_tmp[i]->weight = g->nodesVec[i]->weights[_c];
            }
            else
                if (!isVarFixedToOne(i))
            {
                if (lCosts[i][c] < Sol_nodes_tmp[i]->weight)
                {
                    Sol_nodes_tmp[i]->color = c; //g->nodesVec[i]->colors[c];
                    Sol_nodes_tmp[i]->weight = lCosts[i][c];
                }
            }
        }

        if (Sol_nodes_tmp[i]->color == -1)
        {
            assignments_tmp[i] = 0;
        }
    }
    ...
    // Test if the actual solution is feasible 
    for (int i = 0; i < g->nb_nodes; i++)
    {
        if (!is_dominated(i, Sol_nodes_tmp, assignments_tmp))
            return INFINITY;
    }
    ...
    for (int i = 0; i < g->nb_nodes; i++)
    {
        ...
        if (isVarFixedToOne(i) || assignments_tmp[i] == 0)
            continue;
        ...
        for (int j = 0; j < g->nodesVec[i]->neighbors.size(); j++)
            if (!is_dominated(g->nodesVec[i]->neighbors[j], Sol_nodes_tmp, assignments_tmp))
            {
                assignments_tmp[i] = 1;
                break;
            }
    ...
    for (int i = 0; i < g->nb_nodes; i++)
    {
        ...
        {//cout<< Sol_nodes_tmp[i]->color << "  ";
            val += g->nodesVec[i]->weights[Sol_nodes_tmp[i]->color];

            heuristicSolution[i] = Sol_nodes_tmp[i]->color;
        }
    }//cout<<endl;
    return val;
}

bool Lagrangian::is_dominated(int node, vector < C_assignment *> &Sol_nodes_tmp, vector < bool> &assignments_tmp)
{
    ...
    for (int i = 0; i < g->nodesVec[node]->neighbors.size(); i++)
    {
        if (assignments_tmp[g->nodesVec[node]->neighbors[i]] == 1 && std::find(g->nodesVec[node]->colors.begin(), g->nodesVec[node]->colors.end(),
                g->nodesVec[g->nodesVec[node]->neighbors[i]]->colors[Sol_nodes_tmp[g->nodesVec[node]->neighbors[i]]->color]) != g->nodesVec[node]->colors.end())
            return true;
    }
    return false;
}

int Lagrangian::solveSacaDos()
{ // Solve_Lower_Knapsack(int __Capacity, vector < int > & weights, vector < int > & profits, vector < int > & Set_items) {
    ...
    for (int i = 0; i < g->nb_nodes; i++)
        for (int c = 0; c < g->nodesVec[i]->nb_colors; c++)
        {
            ...
            assignment->node = i;
            assignment->color = c;
            assignment->weight = vColorNeighborhood[i][c].size() + 1;
            assignment->profit = lCosts[i][c];
            Sol_nodes_tmp.push_back(assignment);

            _Capacity += assignment->weight;
            Value_max += assignment->profit;
        }

    int nb_object = Sol_nodes_tmp.size();

    vector<int > selected_node_color(g->nb_nodes, -1);

    _Capacity = _Capacity - g->nb_nodes;


    vector <vector <double > > matrix;
    for (int i = 0; i <= nb_object; i++)
    {
        vector <double> tmpVec(_Capacity + 1, 0.);
        matrix.push_back(tmpVec);
        tmpVec.clear();
    }

    for (int i = 1; i <= nb_object; i++)
    {
        for (int w = 0; w <= _Capacity; w++)
        {
            if (Sol_nodes_tmp[i - 1]->weight <= w)
            {
                if ((Sol_nodes_tmp[i - 1]->profit + matrix[i - 1][w - Sol_nodes_tmp[i - 1]->weight ]) > matrix[i - 1][w])
                {
                    matrix[i][w] = Sol_nodes_tmp[i - 1]->profit + matrix[i - 1][w - Sol_nodes_tmp[i - 1]->weight ];
                }
                else
                {
    ...
    int optimal_value = matrix[ nb_object ][_Capacity ];
    ...
    for (int j = 0; j < Sol_nodes_tmp.size(); j++)
        xSolDualLag[i] = -1;

    for (int j = 0; j < not_selected.size(); j++)
        if (not_selected[j] == 0)
        {
            xSolDualLag[i] = selected_node_color[i];
        }
...
string Lagrangian::statusToString(LAGRANGIAN_STATUS _status) const
{
    ...
    switch (_status)
    {
        case LAG_OPTIMAL: tmp += "LAGRANGIAN_OPTIMAL";
            break;
        case LAG_FEAS_MAX_ITER: tmp += "LAGRANGIAN_FEASIBLE_MAX_ITER";
            break;
        case LAG_FEAS_SMALL_STEPSIZE: tmp += "LAGRANGIAN_FEASIBLE_STEP_SIZE";
            break;
        case LAG_INFEAS: tmp += "LAGRANGIAN_INFEASIBLE";
            break;
        default: tmp += "UNKNOWN";
    }
