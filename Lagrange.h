...
public:
	...
	bool isVarFixedToOne(int _v) const { return fixedToOneVars[_v] != -1; }
	bool isVarFixedToZero(int _v, int _c) const { return isVarFixedToOne(_v) || fixedToZeroVars[_v][_c]; }
	bool isConsInactive(int _v) const { return inactiveCons[_v] > 0; }
	...
	double getBestUb() const { return bub; }
	double getBestLb() const { return blb; }
	void getXSol(vector<int> & _destination) { copy(bestXSol.begin(), bestXSol.end(), _destination.begin()); }

	double getRatio(int _v, int _c);

	bool hasColor(int _v, int _c) { return nodeColorDominance[_v][_c]; }
...
protected:
	...
	double lagrangianHeuristic(double _rclParam);
